...
class Agent(FastAPI):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        max_calls_env = os.getenv("BRAIN_AGENT_MAX_CONCURRENT_CALLS")
        if max_calls_env:
            try:
                ...
                log_warn(
                    f"Invalid BRAIN_AGENT_MAX_CONCURRENT_CALLS='{max_calls_env}', defaulting to {default_limit}"
                )
        else:
            self._max_concurrent_calls = default_limit
    ...
    def handle_serverless(self, event: dict) -> dict:
        ...
        Special Endpoints:
            - /discover: Returns agent metadata for Brain server registration
        ...
        # Auto-register with Brain if needed (for execution requests)
        if self.auto_register and not self.brain_connected:
            try:
                # Attempt registration (non-blocking)
                self.brain_handler._register_agent()
    ...
    def _handle_discovery(self) -> dict:
        ...
        for automatic registration with the Brain server.
